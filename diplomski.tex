...
\begin{figure}[htb]
\centering
\includegraphics[width=8cm]{img/heksakopterMJX1.jpg}
...
\begin{subfigure}{.48\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{img/dron_gore1.jpg}
  \caption{Gornji dio - vidljive svjetleće diode}
  \label{fig:gornji}
\end{subfigure}
\begin{subfigure}{.48\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{img/dron_dole1.jpg}
...
\begin{minipage}{.4\textwidth}
  \centering
  \includegraphics[width=.5\linewidth]{img/wiimote1.jpg}
  \captionof{figure}{Wii Remote}
  \label{fig:Wii Remote}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{img/pixy1.jpg}
...
\begin{minipage}{.6\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{img/modificiranje1.jpg}
  \captionof{figure}{Modificiranje kontrolera}
  \label{fig:kontrolerUnutra}
\end{minipage}%
\begin{minipage}{.4\textwidth}
  \centering
  \includegraphics[width=.5\linewidth]{img/modificirani1.jpg}
...
\begin{figure}[htb]
\centering
\includegraphics[width=11cm]{img/spajanje1.jpg}
...
Potrebno je definirati pretplatu \engl{subscriber} na čvor \engl{node} koji postavlja vrijednosti \emph{duty cycle} za generiranje PWM signal. \emph{Subscriber} ima pozivajući funkciju koja kod promjene u čvoru uzima vrijednosti i sprema ih u varijable.
...
Podešavanje parametara PID regulatora na stvarnom sustavu iziskuje vremena. Pošto je radni prostor nedovoljno velik, pa dron izleti iz njega, u ovom poglavlju bit će prikazani simulacijski rezultati. Način na koji bi se ovaj problem mogao eksperimentalno riješiti je fiksirati kretanje drona na samo jednu os.
Za simulaciju upravljanja drona korišteni je Matlab, odnosno Simulink. Simulink shemom opisani je matematičkim model heksakoptera koji je prikazan na slici \ref{fig:matlab_model}. Matematičko model je opisani izrazima \ref{eq:X} - \ref{eq:kut3}.
\begin{figure}[htb]
\centering
\includegraphics[width=14cm]{img/matlab_model.jpg}
\caption{Matlab shema matematičkog modela}
\label{fig:matlab_model}
\end{figure}
Korišteni parametri za simulaciju prikazani su tablicom \ref{tbl:parametriDrona}.
\begin{table}[htb]
\caption{Parametri drona}
\label{tbl:parametriDrona}
\centering
\begin{tabular}{lcccr} \toprule
Masa [kg] & Udaljenost motora [m] & $I_{xx}$ & $I_{yy}$ & $I_{zz}$\\ \midrule
$0,75$ & $0,25$ & $0,019688$ & $0,019688$ & $0,039380$ \\ \bottomrule
\end{tabular}
\end{table}

Za upravljanje se koriste PID regulatori. Shema cijelog sustava za praćenje trajektorije prikazana je na slici \ref{fig:matlab_control}.
\begin{figure}[htb]
\centering
\includegraphics[width=15cm]{img/matlab_control.jpg}
\caption{Matlab shema praćenja trajektorije}
\label{fig:matlab_control}
\end{figure}
Ugađanjem parametara PID regulatora dolazi se do vrijednosti prikazanih u tablici \ref{tbl:parametriPID}.
\begin{table}[htb]
\caption{Korišteni parametri PID regulatora}
\label{tbl:parametriPID}
\centering
\begin{tabular}{cccc} \toprule
Upravljačka veličina& $\mathbf{K_P}$ & $\mathbf{K_I}$ & $\mathbf{K_D}$\\ \midrule
$\mathbf{U_1}$ & $10$ & $5$ & $10$ \\
$\mathbf{U_2}$ & $5$ & $1$ & $1$ \\
$\mathbf{U_3}$ & $5$ & $1$ & $1$ \\
$\mathbf{U_4}$ & $5$ & $1$ & $1$ \\ \bottomrule
\end{tabular}
\end{table}

Na kraju je potrebno definirati trajektoriju. Napravljene su dvije simulacije. Razlika između njih je što se zadana orijentacija, odnosno kut zavrtanja \engl{yaw}, iz $0$ promijeni u $pi/16 [rad]$ odnosno $11,25 [\deg]$.
Dron se u $t=0 s$ po Z osi diže na 2 m, a u $t=5 s$ na 3 m. Za referencu po X i Y osi zadaje se sinusoida, amplitude 5. Početna pozicija drona je u [5,0,0], što znači da će trajektorija bit kružnica oko točke (0,0) gledajući X-Y ravninu.
Sa slikama \ref{fig:mjerenje1_XYZ} prikazana je razlika između reference i pozicije po X, Y i Z osima dobivene preko matematičkog modela. Prikazano je samo jedno mjerenje jer su razlike gotovo identične. 
\begin{figure}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=13cm]{img/mjerenje1_X.png}
\caption{X os}
\label{fig:mjerenje1_X}
\end{subfigure}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=13cm]{img/mjerenje1_Y.png}
\caption{Y os}
\label{fig:mjerenje1_Y}
\end{subfigure}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=13cm]{img/mjerenje1_Z.png}
\caption{Z os}
\label{fig:mjerenje1_Z}
\end{subfigure}\\[1ex]
\caption{Praćenje trajektorije po X, Y i Z osi}
\label{fig:mjerenje1_XYZ}
\end{figure}
Izlazne vrijednosti iz matematičkog modela, odnosno vanjske petlje upravljanja, su kutovi valjanja, nagibanja i zavrtanja. Na slikama \ref{fig:mjerenja_kutevi} su prikazani ti kutovi za dvije simulacije. Sa slike su vidljive male promjene kod kutova valjanja i nagibanja zbog male promjene kuta zavrtanja. 
\begin{figure}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=13cm]{img/mjerenje1_kut.png}
\caption{Mjerenje 1}
\label{fig:mjerenje1_kut}
\end{subfigure}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=13cm]{img/mjerenje2_kut.png}
\caption{Mjerenje 2 - promjena kuta zavrtanja}
\label{fig:mjerenje2_kut}
\end{subfigure}\\[1ex]
\caption{Kutovi valjanja, nagibanja i zavrtanja za dvije simulacije}
\label{fig:mjerenja_kutevi}
\end{figure}
Praćenje trajektorije po X, Y i Z osi sa slike \ref{fig:mjerenje1_XYZ} u 3D prostoru prikazana je slikom \ref{fig:mjerenje_3D}. Sa ove slike je vidljivo da simulirana trajektorija prati referencu uz neki konstantanu pogrešku zbog toga jer se referenca konstantno mijenja pa dron ima premalo vremena da stigne u tu poziciju. Može se uočiti da prilikom pokretanja, kad je dron lebdio u zraku, ima drugačiju putanju nego prilikom konstantnog kretanja po kružnici. To se javlja zbog momenta inercije. Možemo zaključiti da projektirani sustav dobro prati trajektoriju. Ovime se vidi da bi se dobrim podešavanjem PID kontrolera u eksperimentu mogla pratiti trajektorija. 
\begin{figure}
\centering
\includegraphics[width=15cm]{img/mjerenje2_3D.png}
\caption{Praćenje trajektorije u 3D koordinatnom sustavu}
\label{fig:mjerenje_3D}
\end{figure}
